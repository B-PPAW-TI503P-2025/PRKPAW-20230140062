    ...
    const handleAction = async (endpoint) => {
        ...
        try {
            ...
            const response = await axios.post(
                `http://localhost:3001/api/presensi/${endpoint}`,
                {},
                config
            );
            setMessage(response.data.message);
        } catch (err) {
            setError(err.response ? err.response.data.message : `${endpoint} gagal`);
        }
    };
    
    const handleCheckIn = () => handleAction("check-in");
    const handleCheckOut = () => handleAction("check-out");
